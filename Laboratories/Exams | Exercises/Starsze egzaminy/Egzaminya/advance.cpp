...
template<typename InputIterator> void przesun(InputIterator& itr, int dist, std::random_access_iterator_tag){
    itr += dist;
}
template<typename InputIterator> void przesun(InputIterator& itr, int dist, std::bidirectional_iterator_tag){
    if(dist >=0){
        while(dist>0){
            itr++;
            dist--;
