...
namespace OOP 
{
	...
	inline bool operator<(const CFraction &a, const CFraction &b)
	{
		return (a.GetL() * b.GetM() < a.GetM() * b.GetL()) ? true : false;
	}

	inline CFraction operator*(const CFraction &a, const CFraction &b)
	{
		return CFraction(a.GetL() * b.GetL(), a.GetM() * b.GetM());
	}
